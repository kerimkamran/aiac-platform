...
export default async function AdminAuditPage({
  ...
  const qs = (page: number) => `/admin/audit?${new URLSearchParams({ module, q, from, to, page: String(page) })}`;
  ...
  const columns: DataTableColumn<EventRow>[] = [
    {
      key: "when",
      label: "When",
      render: (e) => <span className="text-muted whitespace-nowrap text-xs">{new Date(e.created_at).toLocaleString()}</span>,
    },
    {
      key: "actor",
      label: "Actor",
      render: (e) => {
        ...
        return (
          <span className="flex items-center gap-2">
            ...
            <span className="font-medium text-foreground text-xs">{actor?.full_name || "System"}</span>
          </span>
        );
      },
    },
    { key: "module", label: "Module", render: (e) => <span className="text-muted">{e.module || "—"}</span> },
    { key: "action", label: "Action", render: (e) => <span className="text-foreground font-medium">{e.action.replace(/_/g, " ")}</span> },
    {
      key: "target",
      label: "Target",
      render: (e) => (
        <span className="text-muted text-2xs">{e.target_type ? `${e.target_type} ${String(e.target_id || "").slice(0, 8)}` : "—"}</span>
      ),
    },
    { key: "ip", label: "IP", render: (e) => <span className="text-muted text-2xs">{e.ip || "—"}</span> },
    {
      key: "result",
      label: "Result",
      render: (e) => (
        <span className={`text-2xs font-bold ${e.result === "denied" ? "text-critical" : e.result === "error" ? "text-warning" : "text-good"}`}>
          {e.result || "success"}
        </span>
      ),
    },
  ...
  return (
    <div className="p-6 lg:p-10 max-w-6xl">
      ...
      <DataTablePagination pageNum={pageNum} totalPages={totalPages} makeHref={qs} />
    </div>
  );
}
