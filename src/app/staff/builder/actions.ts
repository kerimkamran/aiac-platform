...
async function loadEngine(
  ...
  if (!engine || !engine.enabled || !engine.api_key) {
    throw new Error(
      `The ${engineKey === "claude" ? "Claude" : "Perplexity"} engine isn't configured. Add an API key and enable it in Settings first.`
    );
  }
...
export async function generateDefaultAssessment(category: "Core" | "Leadership") {
  ...
  try {
    ...
    let engineKey: "claude" | "perplexity";
...
export async function generateCustomAssessment(formData: FormData) {
  ...
  const engineKey = String(formData.get("engine") || "") as "claude" | "perplexity";
  ...
  try {
    ...
    newId = await insertGeneratedAssessment(supabase, {
      title,
      description: `Generated by ${fullName} using ${engineKey === "claude" ? "Claude" : "Perplexity"}.`,
      mode: "generated",
      engine: engineKey,
      generatedBy: userId,
      competencies: comps || [],
